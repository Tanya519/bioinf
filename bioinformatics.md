## Конспект лекций по биоинформатике, МКН, Осень 2021
...
Будем сканировать все окна, в них искать частотные слова и выбирать наиболее подходящие.

k-mer формирует **(L, t) - clump** в геноме, если существует короткий участок длины L в котором этот k-mer встречается >
= t раз
...
Сканируем окном ширины L, запускаем поиск частотных слов и выдаем нужные k-mers
...
Давайте окном 100k нуклеотидов посчитаем частоту вхождения нуклеотидов
...
Нити ДНК ориентированы. Процесс репликации происходит несимметрично. ДНК полимераза, отвечающая за наращивание нити
может перемещаться только в направлении 3->5. Лидирующие полунити (как раз расположенные в нужном направлении) могут
...
С запаздывающими нитями полимераза работает по-другому, достраивает okazaki-фрагменты по одному, сдвигаясь назад в
нужном направлении.

![img](http://bioinformaticsalgorithms.com/images/Replication/okazaki.png)

Когда ДНК спираль расплетается, возрастает вероятность мутаций. Таким образом, если один из нуклеотидов имеет повышенный
...
Из-за этого происходит дизбаланс в G-C парах до и после ori.
...
По мотивам мы можем построить profile. Но и по profile можем построить motifs, выбрав в каждой строке наиболее вероятный
вариант. Так можно итерироваться бесконечно.
...
Алгоритм может работать, потому что мы ищем наши мотивы в тех строках, в которых ожидаем их увидеть.
...
Идея в том, чтобы в случайном алгоритме модифицировать не все k-mers за раз, а только один. Это позволит плавнее
...
**k-mer Composition** - коллекция k-mer-ов которые могут быть получены по данной строке
...
- **Input:** k-mers
- **Output:** text, which k-mer composition is equal to given

**Граф перекрытий**: каждый read положим в вершину, между вершинами проведем ребро если read-ы можно поставить друг за
другом (ребра направленные)

Задача сборки свелась к поиску гамильтонова пути в таком графе.  (NP hard :`(  )
